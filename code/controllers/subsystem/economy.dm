...
/datum/controller/subsystem/economy/proc/price_update()
	...
	earning_report = "<b>Sector Economic Report</b><br><br> Sector vendor prices is currently at <b>[SSeconomy.inflation_value()*100]%</b>[fluff_string]<br><br> The station spending power is currently <b>[station_total] Libre</b>, and the crew's targeted allowance is at <b>[station_target] Credits</b>.<br><br>[SSstock_market.news_string]" // DOPPLER EDIT, Credit > Libre
	...
	if(HAS_TRAIT(SSstation, STATION_TRAIT_ECONOMY_ALERTS))
		...
		if (moneybags)
			earning_report += "Our GMM Spotlight would like to alert you that <b>[moneybags.account_holder]</b> is your station's most affulent crewmate! They've hit it big with [moneybags.account_balance] Libre saved. " // DOPPLER EDIT, Credit > Libre
			update_alerts = TRUE
